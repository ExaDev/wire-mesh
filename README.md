...
`ts/packages/node` exists: a self-hostable, no-cloud LAN counterpart to `cloudflare-hub`, serving the identical relay-hub domain logic over a plain Node `ws` WebSocket transport instead of a Cloudflare Durable Object — no account, no deployment pipeline, just `npx wire-mesh-node` on whichever machine is already reachable on the network.

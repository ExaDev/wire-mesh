...
- **Authenticated peer discovery.** Every gossiped `peer-advert` is signed by the device it names and carries that device's own key, so it is self-certifying in the same way a capability token is: a receiver checks one with no prior contact with the advertised device and no directory to consult. This is what lets an advert travel beyond its author, which it must, since a hub re-broadcasts what it receives and a gateway forwards the adverts of the local peers it fronts. A hub is a facilitator for the directory, never an authority over it, so a receiver verifies each advert itself rather than trusting whoever forwarded it, and freshness (`snapshot-seconds`) decides which connection a device is routed over so a replayed advert cannot capture its traffic.
...
ts/
  packages/
    ...
    node/                 — a self-hostable, no-cloud LAN counterpart to cloudflare-hub, published to npm
    ...
    wire-mesh-sfu/        — an SFU (media-aware relay) for group audio/video calls, wrapping mediasoup
```
...
`ts/packages/node` exists: a self-hostable, no-cloud LAN counterpart to `cloudflare-hub`, serving the identical relay-hub domain logic over a plain Node `ws` WebSocket transport instead of a Cloudflare Durable Object — no account, no deployment pipeline, just `npx wire-mesh` on whichever machine is already reachable on the network.

`ts/packages/wire-mesh-sfu` exists: an SFU (a media-aware relay for group audio/video calls, a role `relay-hub` is deliberately blind-to-payload and cannot fill). A client's own signaling to it is ordinary, unmodified `core/webrtc`; the one real gap `core/webrtc` alone didn't cover is `webrtc.sfu-track-map` (`spec/webrtc.cddl`), mapping an SDP mid on a client's own connection back to which room member's track it carries, spec-level rather than backend-specific so any implementation of it interoperates with any wire-mesh client by construction. The one backend this package ships wraps [mediasoup](https://mediasoup.org) (an audited RTP-forwarding implementation, the same library-over-reimplementation reasoning FROST threshold signing already settled) behind a provider-neutral `SfuMediaBackend` contract a second backend could satisfy later with no changes to it or its callers.

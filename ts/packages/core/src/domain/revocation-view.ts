...
/**
 * An in-memory RevocationCheck fed by ingested revocation-announce frames. Keyed by token-id alone, per management.cddl's contract: a token-id can legitimately carry multiple recorded entries from different issuers (the token's own issuer, and/or any number of parties holding a delegated manage:revoke authorization over it) -- entriesFor returns every one of them unfiltered, since checking which actually apply (issuer-match, or a verified authorization) is verifyTokenChain's own obligation, not this store's.
 */
  ...
  return {
    async entriesFor(tokenId) {
      return Promise.resolve(
        entriesByTokenId.get(revocationKey(tokenId)) ?? [],
      );
    },

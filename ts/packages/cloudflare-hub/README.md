...
The hub domain logic itself is core's (`wire-mesh-core/domain/relay-hub`), deliberately thin and transport-agnostic so it runs unchanged over core's TCP adapter in tests: pairing `relay-connect` initiators with gossiped targets and forwarding `relay-data` both ways, tearing down a pairing when either endpoint disconnects, and moving a device's mapping when a newer gossip arrives on a fresher connection. This deployment gives it the verification half of the Web Crypto identity adapter, since every advert is checked against the key it carries before the device it names is registered, forwarded, or replayed in a catch-up frame; the hub holds no signing identity of its own, because an advert is self-certifying and nothing here ever signs one. A hub is a facilitator for the directory rather than an authority over it, so a client verifies each advert independently too: nothing a client believes about who is who rests on this deployment behaving. Its `exportConnection`/`restoreConnections` pair is what lets this package carry that state across an eviction without the hub itself knowing anything about sockets or attachments.

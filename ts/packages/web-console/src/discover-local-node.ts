// Same-device node auto-discovery, mirroring agent-comms' own probeLocalMesh shape (mesh-client.ts's isLocal check + a localhost probe) -- but wire-mesh-node has no port walk-up of its own (a single fixed default, override only via its own --bind flag), so this probes exactly one port rather than climbing a range the way agent-comms' bridge does.

/** wire-mesh-node's own documented default bind port (ts/packages/node/src/server.ts's DEFAULT_BIND_ADDRESS) -- the one port this module ever probes. */
export const WIRE_MESH_NODE_DEFAULT_PORT = 8787;
...
/** Probes wire-mesh-node's default port on localhost with a plain HTTP fetch (any non-Upgrade request gets a health response -- see server.ts), the same "fetch is simpler and avoids a visible WS error in the console" approach agent-comms' probeLocalMesh uses. Resolves to the node's ws:// address on any response, or undefined if nothing answered. */
export async function probeLocalNode(
  port: number = WIRE_MESH_NODE_DEFAULT_PORT,

 ...
 *
 * Three of the five packages here are private (`@exadev/wire-mesh-cloudflare-hub`, `@exadev/wire-mesh-web-console`, `wire-mesh-sfu`). They still take part: @semantic-release/npm skips publishing a private package but the version bump, tag, and GitHub Release still happen, and that is what a dependent needs. `@exadev/wire-mesh-web-console`'s build output is copied into wire-mesh's own dist, so wire-mesh has to republish when the console changes -- with commits path-filtered per package, the only thing that forces it is web-console releasing and the orchestrator rewriting wire-mesh's dependency range on it.
 */
const config: Pick<
